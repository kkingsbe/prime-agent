...
2. WRITE `PLAN.md` in this directory: at least 3 numbered tasks, one per line:
...
3. DELEGATE one child per task — one delegate call per task, sequential. Name
   each child after its task (T1, T2, T3). Give the child its task text verbatim.
